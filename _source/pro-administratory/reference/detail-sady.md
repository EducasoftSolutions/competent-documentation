...
Detail sady je obrazovka, která zobrazuje informace o aktivitě typu **Sada** – kontejneru sdružujícím více potomků (dílčích aktivit) do jednoho celku. Stránka popisuje, čím se Detail sady liší od detailu běžné aktivity; pro sdílené části odkazuje na [Detail aktivity](detail-aktivity.md).
...
Variantou Sady je **Termínová sada** – varianta sady s termíny (přidává tab **Termíny**). Termínová sada je popsána samostatně: [Termínová sada](../concepts/terminova-sada.md).
...
!!! warning "Dvojklik neotevře detail"
...
| **Detaily** | Formulář s vlastnostmi Sady (viz [Detail aktivity](detail-aktivity.md)). |
| **Dokumenty** | Přílohy a studijní materiály spojené se Sadou (viz [Detail aktivity](detail-aktivity.md)). |
| **Podmínky** | Konfigurace podmínek přístupu k Sadě (viz [Detail aktivity](detail-aktivity.md)). |
...
Taby **Detaily**, **Dokumenty** a **Podmínky** fungují u Sady stejným způsobem jako u běžné aktivity. Jejich popis viz [Detail aktivity](detail-aktivity.md).
...
| 3 | **Nastavení plnění** | Nastavení dat přístupu. Horní řádek zobrazuje Sadu jako celek; pod ní jsou uvedeny jednotlivé vybrané potomky s možností nastavit data samostatně. Tlačítko **Uložit** / **Zavřít**. Podrobný popis nastavení viz [Přiřazení uživatelů k aktivitě](../how-to/aktivity/prirazeni-uzivatelu-k-aktivite.md). |

---

## Pozor na

!!! note "Částečné splnění – skrytý parametr"
